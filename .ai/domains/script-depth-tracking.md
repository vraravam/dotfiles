...
External tools (`git`, `mise`, `sqlite3`, `keybase`, `gpg`, etc.) invoked via `system()` or `Open3.capture3()` print at column 0. This is intentional -- wrapping their output would add complexity for minimal UX benefit. Tool output remains visually distinct from our structured logging.
