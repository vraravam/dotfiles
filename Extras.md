...
ONE-TIME migration tool: migrates git repositories to the encrypted-backup mechanism (`gpg` + `git bundle`, see [`KEYBASE_MIGRATION.md`](KEYBASE_MIGRATION.md)). Run once per repository when first adopting the mechanism — there is nothing left to "migrate" for that repo afterward; day-to-day pushes go through `EncryptedBackup.export_and_push` instead, wired up via the `push-<basename>.sh` override scripts.
...
With no `--repo`, migrates all known repos (`home`, `browser-profiles`) in a single run. With `--repo` (and `--encrypted-repo-name`), migrates only that repository.

```zsh
# All known repos at once
migrate-repo-to-encrypted-backup.rb
