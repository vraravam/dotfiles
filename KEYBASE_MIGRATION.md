...
This adds a second, independent encrypted-backup mechanism (`scripts/utilities/encrypted_backup.rb`) alongside Keybase, rather than replacing it. If a repo (`~` or `${PERSONAL_PROFILES_DIR}`) still has a `keybase://` `origin` remote, it is left completely untouched and keeps being pushed to normally -- the new mechanism is a side-channel export to a separate plain GitHub repo, not a replacement remote. Running both means two independent, addressable restore points, so you can choose which one to use when setting up a new machine, instead of committing to just one.

### What Changed

**Removed (from this codebase's automation, not from any machine that already has it):**
- Keybase.app's Brewfile cask (no longer auto-installed on a fresh machine -- install it yourself first if you want to keep using Keybase there)
- `scripts/utilities/keybase.rb` and every call site (identity/login-status derivation; a plain `git push`/`git pull` to an existing `keybase://` remote needs no special code beyond Keybase itself being installed and running)
- `KEYBASE_*` environment variables (no longer needed to derive identity -- see `scripts/utilities/keybase.rb`'s removal above)
...
- `scripts/migrate-repo-to-encrypted-backup.rb` -- **ONE-TIME** setup tooling (run once per repo, to add the encrypted backup alongside any existing remote; no `--repo` sets up all known repos in one run, `--repo`/`--encrypted-repo-name` targets a single repo)
- `${PERSONAL_BIN_DIR}/push-<home-basename>.sh` / `push-browser-profiles.sh` -- transparent day-to-day refresh of **both** backups, unconditionally, on every push (routine, runs every push)
...
## Setting Up Encrypted Backup for an Existing Repo (One-Time, Per Repo)

Run this once per repository, to add the encrypted backup alongside anything already
syncing that repo (e.g. a `keybase://` `origin`). Nothing left to set up for that repo
afterward -- day-to-day pushes go through the `push-<basename>.sh` override scripts instead
(see "Day-to-Day Usage" below), which push **both** the encrypted backup and the existing
`origin` (if any) on every run. Re-running this tool is harmless (it just re-bundles,
...
This never touches either repo's existing remotes -- it only bundles, encrypts, and pushes
to the separate wrapper repo. If `origin` points at a `keybase://` URL, it is left exactly
as-is and keeps being pushed to normally by `push` -- that's the whole point: two
independent, addressable backups to choose between on a new machine, not a replacement of
one by the other.
...
Nothing changes. Run `push` and `pull` (the shell functions, not `git push`/`git pull` -- see `git-config.md` for why the distinction matters) in `~` or `${PERSONAL_PROFILES_DIR}` as you always have -- including the sequential push-on-machine1, pull/rebase-on-machine2 workflow Keybase supported. The per-repo override scripts (`push-<home-basename>.sh`/`pull-<home-basename>.sh` -- basename of `$HOME`, typically `$USER` -- and `push-browser-profiles.sh`/`pull-browser-profiles.sh`) automatically refresh/consume the encrypted backup. If `origin` is still a live `keybase://` remote, `push` syncs it too, unconditionally, on every run -- both backups are always kept current, not either/or. See `EncryptedBackup.fetch_and_rebase` (`scripts/utilities/encrypted_backup.rb`) for how `pull` fetches the latest encrypted blob and rebases the current branch onto it, independently of whatever `origin` may or may not be. The existing cron schedule that already runs `push-browser-profiles.sh` keeps that backup current without any extra action.
