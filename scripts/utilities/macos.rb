...
module MacOS
  ...
  # Starts a background thread that runs 'sudo -v' every 60 seconds to keep sudo
  # credentials alive for the remainder of the running script -- mirrors
  # keep_sudo_alive in .shellrc. Standalone entry point for callers (e.g.
  # fresh-install-of-osx.rb) that only need the keep-alive behavior, without the
  # side effect of also disabling the software-update schedule that
  # suspend_softwareupdate_schedule below carries. Delegates to the same private
  # _keep_sudo_alive helper suspend_softwareupdate_schedule/resume_softwareupdate_schedule
  # use internally, guarded by @_sudo_alive_running so only one thread ever runs
  # regardless of which caller started it first.
  #
  # @return [void]
  def keep_sudo_alive
    _keep_sudo_alive
  ...
  private_class_method :_process_running?, :_set_softwareupdate_schedule,
                       ...
                       :_first_install_brewfile_content
end
