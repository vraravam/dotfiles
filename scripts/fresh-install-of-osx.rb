...
def setup_jio_dns
  ...
  unless CommandUtils.run_silent('networksetup', '-setdnsservers', 'Wi-Fi', '1.1.1.2', '9.9.9.9')
    warn 'Failed to set DNS for Wi-Fi'
  end
end

# Downloads .shellrc from GitHub (on FIRST_INSTALL) and sources it.
# Mirrors _download_and_source_shellrc in the shell version.
def download_and_source_shellrc(curl_opts, cache_bust_headers)
  Logging.section_header("Download the '~/.shellrc' for loading the utility functions")
  ...
  if EnvVars.first_install?
    ...
    cmd = ['curl'] + cache_bust_headers + curl_opts + ['-fsSL', url, '-o', shellrc_path.to_s]
    unless system(*cmd)
      error 'Failed to download .shellrc'
    end
    ...
    unless shellrc_path.file? && shellrc_path.size.positive? && shellrc_path.read.include?('is_shellrc_sourced')
      error "Downloaded .shellrc appears corrupted or empty"
    end

    success "Successfully downloaded '#{shellrc_path.to_s.cyan}'"
  else
    # Pre-configured OS: skip downloading; the built-in guard makes the source below a no-op if already loaded.
    info "Skipping downloading '#{shellrc_path.to_s.yellow}' since this is not a first install"
  end

  # Source .shellrc with DEBUG=true to show which config files load.
  # Ruby doesn't have zsh's 'unfunction' or 'source' built-ins, so we call zsh.
  # The guard function check is handled inside .shellrc itself on subsequent calls.
  system({ 'DEBUG' => 'true' }, 'zsh', '-c', "source #{shellrc_path.to_s.shellescape}")
  success "Successfully sourced '#{shellrc_path.to_s.cyan}'"
end

# Validates that curl-downloaded .shellrc matches the repo version.
# Mirrors validation logic at lines 569-584 in the shell version.
def validate_shellrc_matches_repo
  ...
  stdout, stderr, status = Open3.capture3('/usr/bin/diff', '-q', shellrc_home.to_s, shellrc_repo.to_s)

  return if status.success? # Files match, validation passed

  # Files differ -- GitHub cache is stale
  $stderr.puts "ERROR: [FIRST_INSTALL] The curl-downloaded ~/.shellrc differs from the repo version."
  $stderr.puts "This indicates GitHub's raw.githubusercontent.com cache is stale."
  $stderr.puts ""
  $stderr.puts "Diff output:"
  diff_output, = Open3.capture3('/usr/bin/diff', '-u', shellrc_home.to_s, shellrc_repo.to_s)
  $stderr.puts diff_output.lines.first(50).join
  $stderr.puts ""
  $stderr.puts "Wait 5-10 minutes for the cache to refresh, then re-run this script."
  $stderr.puts "Alternatively, manually copy the repo version:"
  $stderr.puts "  cp '#{shellrc_repo}' '#{shellrc_home}'"
  $stderr.puts "  source '#{shellrc_home}'"
  $stderr.puts "  #{$PROGRAM_NAME} #{ARGV.join(' ')}"
  exit 1
end

# Restores .shellrc from git after install-dotfiles.rb moves the downloaded version.
# Mirrors restore logic at lines 594-601 in the shell version.
def restore_shellrc_after_install_dotfiles
  return unless EnvVars.first_install?
  ...
  stdout, stderr, status = git.diff('--quiet', '--', shellrc_relative)
  return if status.success? # No changes, nothing to restore

  # Restore committed version
  git.checkout('--', shellrc_relative)

  # Force re-source the restored version
  # Ruby doesn't have zsh's unfunction/source, so call zsh
  shellrc_path = EnvVars::HOME.join('.shellrc')
  system({ 'DEBUG' => 'true' }, 'zsh', '-c', "source #{shellrc_path.to_s.shellescape}")
end
...
def clone_home_repo
  Logging.with_step('Clone home repo', "Cloning 'home' repo") do
    ...
    if GitProcessor.repo?(EnvVars::HOME)
      ...
      _stdout, _stderr, status = git.pull(rebase: true)
      if status.success?
...
Cron.with_cron_suspended do
  ...
  download_and_source_shellrc(curl_opts, cache_bust_headers)
  ...
  restore_shellrc_after_install_dotfiles
  ...
  if EnvVars.first_install?
    Logging.with_step('Unshallow dotfiles repo', 'Pulling full history for dotfiles') do
      git = GitProcessor.new(dir: EnvVars::DOTFILES_DIR)
      # Run unshallow first (converts shallow → full clone and configures remote branches)
      _stdout, _stderr, status = git.run_alias('unshallow')
      unless status.success?
        record_warning "Failed to unshallow dotfiles repo -- skipping pull"
        next
      end
      # Then fetch to retrieve complete history for all branches
      _stdout, _stderr, status = git.fetch
      if status.success?
        success 'Successfully unshallowed and fetched dotfiles repo'
      else
        record_warning "Unshallowed but failed to fetch dotfiles repo -- run manually: cd '#{EnvVars::DOTFILES_DIR.to_s.cyan}' && git fetch"
      end
    end
  end
  ...
  if EnvVars.first_install?
    Logging.user_action "Remaining repositories were cloned shallow (--depth=1) to save time. Run '#{'all unshallow'.yellow}' to convert them to full clones, then run '#{'all fetch'.yellow}' or '#{'all pull'.yellow}' to retrieve complete history for all branches."
  end
