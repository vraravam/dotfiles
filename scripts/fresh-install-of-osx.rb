...
# 'origin2' directly here instead, leaving 'origin' free for Keybase to claim.
#
# @param keybase_repo_name [String]
# @return [String] 'origin' or 'origin2'
def _encrypted_backup_remote_name(keybase_repo_name)
  nil_or_empty?(keybase_repo_name) ? 'origin' : 'origin2'
end
...
# fallback, or the only option if Keybase isn't enabled/available. Whichever
# succeeds performs the actual clone; if the other mechanism is also enabled, it is
# configured as an additional remote (via _configure_backup_remote) rather than
# cloned from again.
    ...
    if already_exists
      ...
      unless nil_or_empty?(keybase_repo_name)
        if PathUtils.command_exists?('keybase') && Keybase.ensure_logged_in &&
          ...
          record_warning 'Failed to clone home repo from Keybase -- will try encrypted-backup next if enabled'
        end
      end

      if nil_or_empty?(cloned_via) && !nil_or_empty?(encrypted_repo_url)
        if PathUtils.command_exists?('git-gpg-encrypt-restore') && system('git-gpg-encrypt-restore', encrypted_repo_url, home.to_s)
          cloned_via = 'encrypted-backup'
          # A bundle-based import ('git clone <bundle-file>') never creates an 'origin'
          # remote at all (confirmed empirically: 'git remote get-url origin' errors "No
          # such remote" immediately after) -- 'add_remote' here, not a URL update on an
          # existing remote. See _encrypted_backup_remote_name's doc for why the remote
          # name isn't always 'origin'.
          remote_name = _encrypted_backup_remote_name(keybase_repo_name)
          GitProcessor.new(dir: home).add_remote(remote_name, "gpg-encrypt::#{encrypted_repo_url}")
          success 'Successfully cloned home repo from encrypted backup'
        else
          record_error 'Failed to clone home repo from encrypted backup'
        end
      end
      ...
      elsif !nil_or_empty?(keybase_repo_name) || !nil_or_empty?(encrypted_repo_url)
        record_error 'Failed to clone home repo from any enabled backup mechanism'
      else
        info "Skipping cloning of home repo since neither 'KEYBASE_HOME_REPO_NAME' nor 'ENCRYPTED_HOME_REPO_URL' env var has been set"
      end
    end
    ...
    if nil_or_empty?(profiles_dir.to_s)
      ...
      unless nil_or_empty?(keybase_repo_name)
        if PathUtils.command_exists?('keybase') && Keybase.ensure_logged_in &&
          ...
          record_warning 'Failed to clone browser-profiles repo from Keybase -- will try encrypted-backup next if enabled'
        end
      end

      if nil_or_empty?(cloned_via) && !nil_or_empty?(encrypted_repo_url)
        if PathUtils.command_exists?('git-gpg-encrypt-restore') && system('git-gpg-encrypt-restore', encrypted_repo_url, profiles_dir.to_s)
          cloned_via = 'encrypted-backup'
          # See _clone_home_repo's matching comment -- a bundle-based import never
          # creates an 'origin' remote at all, so 'add_remote' (not a URL update).
          remote_name = _encrypted_backup_remote_name(keybase_repo_name)
          GitProcessor.new(dir: profiles_dir).add_remote(remote_name, "gpg-encrypt::#{encrypted_repo_url}")
          success 'Successfully cloned browser-profiles repo from encrypted backup'
        else
          record_error 'Failed to clone browser-profiles repo from encrypted backup'
        end
      end

      if nil_or_empty?(cloned_via)
        if !nil_or_empty?(keybase_repo_name) || !nil_or_empty?(encrypted_repo_url)
          record_error 'Failed to clone browser-profiles repo from any enabled backup mechanism'
        else
          info "Skipping cloning of profiles repo since neither 'KEYBASE_PROFILES_REPO_NAME' nor 'ENCRYPTED_PROFILES_REPO_URL' env var has been set"
        end
      end
    end
...
Cron.with_cron_suspended do
  ...
  if EnvVars::PERSONAL_CONFIGS_DIR.directory?
    ...
    if failed_files.empty?
      ...
      record_error "Failed to process #{failed_files.count} file(s): #{failed_files.join(', ')}"
    end
  else
