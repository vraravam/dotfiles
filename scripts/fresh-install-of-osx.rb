...
def restore_shellrc_after_install_dotfiles
  ...
  _stdout, _stderr, status = git.diff('--quiet', '--', shellrc_relative)
...
def configure_dot_files_repo
  ...
  Logging.with_step('Configure dotfiles repo', "Configuring dotfiles repo at '#{dotfiles_dir.to_s.cyan}'") do
    ...
    unless git.repo?
      if EnvVars.first_install?
        ...
        backup_dir = dotfiles_dir.parent.join("#{dotfiles_dir.basename}-tarball-backup")
        backup_dir_str = backup_dir.to_s
        backup_dir.rmtree if backup_dir.exist?
        FileUtils.mv(dotfiles_dir.to_s, backup_dir_str, force: true)

        # Clone proper git repo
        url = "https://github.com/#{EnvVars::GH_USERNAME}/dotfiles"
        branch = EnvVars::DOTFILES_BRANCH
        unless GitProcessor.clone_repo_into(url, dotfiles_dir, branch: branch)
          # Restore backup if clone failed
          dotfiles_dir.rmtree if dotfiles_dir.exist?
          FileUtils.mv(backup_dir_str, dotfiles_dir.to_s, force: true)
          error "Failed to clone dotfiles repo from '#{url}'"
        end

        # Remove backup if clone succeeded
        FileUtils.rm_rf(backup_dir_str)
        success 'Successfully converted tarball to git repository'
      else
        error "Dotfiles directory '#{dotfiles_dir.to_s.cyan}' is not a git repo"
      end
    end
    ...
    record_warning 'Failed to add upstream git config for dotfiles repo' unless AddUpstreamGitConfig.run(dir: dotfiles_dir, upstream_owner: EnvVars::UPSTREAM_GH_USERNAME)
  end
end
...
Cron.with_cron_suspended do
  ...
  Logging.user_action "Remaining repositories were cloned shallow (--depth=1) to save time. Run '#{'all unshallow'.yellow}' to fetch complete history, then '#{'git rebase @{u}'.yellow}' or '#{'git merge @{u}'.yellow}' in each repo to update working trees." if EnvVars.first_install?
