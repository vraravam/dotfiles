...
def _download_and_source_shellrc(gh_username, curl_opts, cache_bust_headers)
  ...
  if reason
    ...
    url = "https://raw.githubusercontent.com/#{gh_username}/dotfiles/refs/heads/#{EnvVars::DOTFILES_BRANCH}/files/--HOME--/.shellrc?#{timestamp}"
...
def _clone_dot_files_repo(gh_username)
  dotfiles_dir = EnvVars::DOTFILES_DIR
...
def _clone_home_repo
  ...
  home_repo_name = 'home' if nil_or_empty?(home_repo_name)

  Logging.with_step('Clone home repo', "Cloning '#{home_repo_name}' repo") do
    if nil_or_empty?(home_repo_name)
      info "Skipping cloning of home repo since 'KEYBASE_HOME_REPO_NAME' was explicitly set to empty"
      next
    end
...
def _clone_profiles_repo
  ...
  profiles_repo_name = 'profiles' if nil_or_empty?(profiles_repo_name)

  Logging.with_step('Clone profiles repo', "Cloning '#{profiles_repo_name}' repo") do
    if nil_or_empty?(profiles_repo_name) || nil_or_empty?(EnvVars::PERSONAL_PROFILES_DIR.to_s)
      info "Skipping cloning of profiles repo since either 'KEYBASE_PROFILES_REPO_NAME' was explicitly set to empty or 'PERSONAL_PROFILES_DIR' hasn't been set"
      next
    end
...
# Decides whether to sync silently, attempt login, or skip -- then clones the
# home + profiles repos accordingly. Called only when 'keybase' is installed.
def _setup_and_clone_keybase_repos
  if Keybase.username
...
Cron.with_cron_suspended do
  ...
  _download_and_source_shellrc(gh_username, curl_opts, cache_bust_headers)
  ...
  _clone_dot_files_repo(gh_username)
  ...
  # Keybase repos (home + profiles) -- optional, for encrypted preference backups.
  if PathUtils.command_exists?('keybase')
    Logging.with_step('Keybase', 'Keybase (optional, for encrypted preference backups)') do
      _setup_and_clone_keybase_repos
    end
  else
