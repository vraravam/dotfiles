...
PLISTBUDDY = Core::ROOT.join('usr', 'libexec', 'PlistBuddy').to_s.freeze
...
# Module to track script mode (silent vs interactive)
module ScriptMode
  @auto = false
...
# Interactive y/n prompt. In silent mode (ScriptMode.auto?=true) the default is accepted
...
# Failures are recorded as warnings (not fatal) -- unsupported keys on the current OS
# are non-fatal, mirroring (in spirit) the shell version's ERR-trap-based
# _record_error collection, so the final summary still reflects per-setting failures
# instead of always reporting a clean run regardless of what actually happened.
def _d(*args)
  ok = system('defaults', 'write', *args.map(&:to_s))
  Logging.record_warning("defaults write #{args[0]} #{args[1]} failed (unsupported on this OS version?)") unless ok
  ok
end

# Runs: defaults -currentHost write <domain> <key> [type] <value> ...
def _dh(*args)
  ok = system('defaults', '-currentHost', 'write', *args.map(&:to_s))
  Logging.record_warning("defaults -currentHost write #{args[0]} #{args[1]} failed (unsupported on this OS version?)") unless ok
  ok
end

# Runs: sudo defaults write <domain> <key> [type] <value> ...
def _ds(*args)
  ok = system('sudo', 'defaults', 'write', *args.map(&:to_s))
  Logging.record_warning("sudo defaults write #{args[0]} #{args[1]} failed (unsupported on this OS version?)") unless ok
  ok
end
...
# before Add pattern).
# :reek:UtilityFunction -- Stateless PlistBuddy wrapper (correct design)
def _pbd(cmd, file)
  CommandUtils.run_silent(PLISTBUDDY, '-c', cmd, file)
end
...
# This avoids Delete+Add, which leaves the key missing (not merely stale) if the
# script is interrupted between the Delete and the Add.
#
# @param file [String] Plist file path.
# @param key [String] Key path (e.g., ":'Window Settings':'Clear Dark':rowCount").
# @param value [String] Value to set.
# @param type [String] Type for the Add fallback (e.g. 'integer', 'bool', 'string').
# @return [Boolean] true on success, false on failure.
# :reek:UtilityFunction -- Stateless PlistBuddy wrapper (correct design)
def _pb_set_or_add(file, key, value, type)
  return true if CommandUtils.run_silent(PLISTBUDDY, '-c', "Set #{key} #{value}", file)

  _pb("Add #{key} #{type} #{value}", file)
end

private :_ask, :_d, :_dh, :_ds, :_pb, :_pbd, :_pb_set_or_add
...
ScriptMode.auto = options[:silent] || false

# ---------------------------------------------------------------------------
# Entry-point guards

unless ScriptMode.auto? || $stdin.tty? || EnvVars.force_color?
  warn('Interactive mode requires a terminal. Use -s for silent mode.')
  exit 1
end
...
# credentials alive for all subsequent sudo calls in this script. Always runs,
# regardless of silent/auto mode (idempotent -- harmless if already authenticated).
system('sudo', '-v')
...
at_exit do
  # Mirrors the shell version's 'print_script_summary "${script_start_time}" \'Done. ...\''
  # call -- passes the message through to print_script_summary (which now has real
  # collected warnings to show, per the _d/_dh/_ds fix above) instead of a separate,
  # always-shown 'success' call.
  print_script_summary(start_time, 'Done. Note that some of these changes require a logout/restart to take effect.')
...
_ds('/Library/Preferences/com.apple.loginwindow', 'GuestEnabled', '-bool', 'false') if _ask('Disable guest login', 'Y')

# ---------------------------------------------------------------------------
# MenuBar / Control Center

_d('com.apple.controlcenter', 'NSStatusItem Visible Bluetooth', '1')
_d('com.apple.controlcenter', 'NSStatusItem Visible WiFi', '-bool', 'true')
_d('com.apple.controlcenter', 'NSStatusItem Visible Battery', '0')
_d('com.apple.controlcenter', 'NSStatusItem VisibleCC Clock', '-bool', 'false')
# Spotlight = off (use Sol instead)
# _d('com.apple.controlcenter', 'NSStatusItem Visible Spotlight', '-bool', 'false')
_d('com.apple.controlcenter', 'NSStatusItem Visible AirDrop', '-bool', 'false')
_d('com.apple.controlcenter', 'NSStatusItem Visible TextInput', '-bool', 'false')
_d('com.apple.controlcenter', 'NSStatusItem Visible KeyboardBrightness', '-bool', 'false')
_d('com.apple.controlcenter', 'NSStatusItem Visible Weather', '-bool', 'false')
# Focus = show when active (8=when active, 16=always, 24=never)
_d('com.apple.controlcenter', 'FocusModes', '-int', '8')
_d('com.apple.controlcenter', 'AirPlayDisplay', '-int', '8')
_d('com.apple.controlcenter', 'Display', '-int', '8')
_d('com.apple.controlcenter', 'Sound', '-int', '8')
_d('com.apple.controlcenter', 'NowPlaying', '-int', '8')
...
_dh('com.apple.controlcenter', 'KeyboardBrightness', '8') if _ask('Keep keyboard brightness at maximum', 'Y')
...
if _ask('Set computer name (as done via System Preferences → Sharing)', 'Y')
  ...
  username_in_camel_case = EnvVars::USER.split(/[-_\s]/).map(&:capitalize).join
  human_date = Time.now.strftime('%Y-%m-%d-%H-%M')
  ...
  _ds('/Library/Preferences/SystemConfiguration/com.apple.smb.server',
      'NetBIOSName', '-string', "#{username_in_camel_case}-#{human_date}")
end

system('sudo', 'pmset', '-a', 'standbydelay', '21600') if _ask('Set standby delay to 6 hours (default: 1 hour)', 'Y')
...
_d('com.apple.systemuiserver', 'menuExtras', '-array',
...
_d('com.apple.systemuiserver', 'NSStatusItem Visible Siri', '-bool', 'false')
_d('com.apple.systemuiserver', 'NSStatusItem Visible com.apple.menuextra.airport', '-bool', 'true')
_d('com.apple.systemuiserver', 'NSStatusItem Visible com.apple.menuextra.appleuser', '-bool', 'true')
_d('com.apple.systemuiserver', 'NSStatusItem Visible com.apple.menuextra.battery', '-bool', 'true')
_d('com.apple.systemuiserver', 'NSStatusItem Visible com.apple.menuextra.bluetooth', '-bool', 'true')
_d('com.apple.systemuiserver', 'NSStatusItem Visible com.apple.menuextra.volume', '-bool', 'true')

_d('com.apple.menuextra.clock', 'DateFormat', '-string', 'EEE d MMM hh:mm:ss a')
_d('com.apple.menuextra.clock', 'FlashDateSeparators', '-bool', 'true')
# IsAnalog=true because The Clocker app is used for the menu bar clock display.
_d('com.apple.menuextra.clock', 'IsAnalog', '-bool', 'true')
_d('com.apple.menuextra.clock', 'Show24Hour', '-bool', 'false')
_d('com.apple.menuextra.clock', 'ShowAMPM', '-bool', 'true')
_d('com.apple.menuextra.clock', 'ShowDate', '-bool', 'false')
_d('com.apple.menuextra.clock', 'ShowDayOfMonth', '-bool', 'true')
_d('com.apple.menuextra.clock', 'ShowDayOfWeek', '-bool', 'false')
_d('com.apple.menuextra.clock', 'ShowSeconds', '-bool', 'true')
...
_d('-g', 'NSQuitAlwaysKeepsWindows', '-bool', 'true') if _ask('Keep windows open when quitting and re-opening apps (Resume)', 'Y')
...
if _ask('Restart automatically if the computer freezes', 'Y')
  ...
  CommandUtils.run_silent('sudo', 'systemsetup', '-setrestartfreeze', 'on', out: :out)
end

CommandUtils.run_silent('sudo', 'systemsetup', '-settimezone', 'Asia/Calcutta', out: :out) if _ask('Set the timezone to Asia/Calcutta', 'Y')

CommandUtils.run_silent('sudo', 'systemsetup', '-setusingnetworktime', 'on', out: :out) if _ask('Sync time automatically using network time servers', 'Y')

CommandUtils.run_silent('sudo', 'systemsetup', '-setcomputersleep', '10', out: :out) if _ask('Set the computer sleep time to 10 minutes', 'Y')

CommandUtils.run_silent('sudo', 'systemsetup', '-setdisplaysleep', '10', out: :out) if _ask('Set the display sleep time to 10 minutes', 'Y')

CommandUtils.run_silent('sudo', 'systemsetup', '-setharddisksleep', '15', out: :out) if _ask('Set the hard disk sleep time to 15 minutes', 'Y')

_d('-g', 'NSAutomaticCapitalizationEnabled', '-bool', 'false') if _ask('Disable automatic capitalization', 'Y')
...
if _ask('Set preferred languages to English (India, US) and clear recent places', 'Y')
  ...
  CommandUtils.run_silent('defaults', 'delete', 'NSGlobalDomain', 'NSNavRecentPlaces', out: :out)
end
...
_d('-g', 'NSAutomaticPeriodSubstitutionEnabled', '-bool', 'false') if _ask('Disable automatic period substitution (double-space → period)', 'Y')
...
system('sudo', 'pmset', '-a', 'hibernatemode', '0') if _ask('Disable hibernation (speeds up entering sleep mode)', 'Y')

system('sudo', 'pmset', '-a', 'sms', '0') if _ask('Disable the sudden motion sensor (not useful for SSDs)', 'Y')
...
if _ask('Enable trackpad gestures (tap-to-click, three-finger drag, etc.)', 'Y')
  ...
  }.each { |dom, keys| keys.each { |key, args| _d(dom, key, *args) } }
...
if _ask('Apple Multitouch mouse features', 'Y')
  {
    ...
    'UserPreferences' => ['-int', '1']
  }.each { |key, args| _d('com.apple.AppleMultitouchMouse', key, *args) }
end

_d('-g', 'AppleKeyboardUIMode', '-int', '2') if _ask('Enable full keyboard access for all controls (e.g. Tab in modal dialogs)', 'Y')
...
_d('com.apple.finder', 'QuitMenuItem', '-bool', 'true') if _ask('Allow quitting Finder via ⌘Q (also hides desktop icons)', 'Y')
...
_d('com.apple.finder', 'ShowHardDrivesOnDesktop', '-bool', 'false') if _ask('Hide hard drive icons on the desktop', 'N')

_d('com.apple.finder', 'AppleShowAllFiles', '-bool', 'false') if _ask('Hide hidden files by default in Finder', 'N')

_d('-g', 'AppleShowAllExtensions', '-bool', 'true') if _ask('Show all filename extensions', 'Y')

_d('com.apple.finder', '_FXShowPosixPathInTitle', '-bool', 'true') if _ask('Display full POSIX path as Finder window title', 'Y')

_d('com.apple.finder', 'ShowStatusBar', '-bool', 'true') if _ask('Show status bar in Finder windows', 'Y')

_ds('/Library/Preferences/com.apple.finder', 'PathBarRootAtHome', '-bool', 'true') if _ask("Start the status bar path at #{EnvVars::HOME} (instead of 'Hard drive')", 'Y')

_d('com.apple.finder', 'ShowPathbar', '-bool', 'true') if _ask('Show path (breadcrumb) bar in Finder windows', 'Y')

_d('com.apple.finder', 'ShowPreviewPane', '-bool', 'false') if _ask('Hide the preview pane in Finder', 'Y')

_d('com.apple.finder', 'ShowExternalHardDrivesOnDesktop', '-bool', 'true')
_d('com.apple.finder', 'ShowMountedServersOnDesktop', '-bool', 'false')
_d('com.apple.finder', 'ShowRecentTags', '-bool', 'false')
_d('com.apple.finder', 'ShowRemovableMediaOnDesktop', '-bool', 'true')
_d('com.apple.finder', 'ShowSidebar', '-bool', 'true')
_d('com.apple.finder', 'SidebarDevicesSectionDisclosedState', '-bool', 'true')
_d('com.apple.finder', 'SidebarPlacesSectionDisclosedState', '-bool', 'true')
_d('com.apple.finder', 'SidebarShowingSignedIntoiCloud', '-bool', 'true')
_d('com.apple.finder', 'SidebarShowingiCloudDesktop', '-bool', 'true')
_d('com.apple.finder', 'SidebarTagsSctionDisclosedState', '-bool', 'true')
_d('com.apple.finder', 'SidebarWidth', '172')
_d('com.apple.finder', 'SidebariCloudDriveSectionDisclosedState', '-bool', 'true')
_d('com.apple.finder', 'FXRemoveOldTrashItems', '-bool', 'true')
_d('com.apple.finder', '_FXEnableColumnAutoSizing', '-bool', 'true')
# Default view style: clmv=column, icnv=icon, Nlsv=list, glyv=gallery.
_d('com.apple.finder', 'FXPreferredViewStyle', '-string', 'clmv')
_d('com.apple.finder', 'WarnOnEmptyTrash', '-bool', 'false')
_d('com.apple.finder', 'OpenWindowForNewRemovableDisk', '-bool', 'true')
_d('com.apple.finder', 'RestoreWindowState', '-bool', 'true')
...
_d('com.apple.finder', 'QLEnableTextSelection', '-bool', 'true') if _ask('Allow text selection in Quick Look / Preview', 'Y')
...
_d('com.apple.finder', 'FXDefaultSearchScope', '-string', 'SCcf') if _ask('When performing a search, search the current folder by default (not This Mac)', 'Y')

_d('com.apple.finder', 'FXEnableExtensionChangeWarning', '-bool', 'false') if _ask('Disable the warning when changing a file extension', 'N')
...
_d('com.apple.finder', 'EmptyTrashSecurely', '-bool', 'true') if _ask('Empty Trash securely by default', 'Y')

_d('com.apple.finder', 'FK_AppCentricShowSidebar', '-bool', 'true') if _ask('Show app-centric sidebar', 'Y')

system('chflags', 'nohidden', EnvVars::HOME.join('Library').to_s) if _ask("Show the #{EnvVars::HOME}/Library folder", 'Y')

system('sudo', 'nvram', 'boot-args=mbasd=1') if _ask('Enable the MacBook Air SuperDrive on any Mac', 'N')

system('sudo', 'chflags', 'nohidden', '/Volumes') if _ask("Show the '/Volumes' folder", 'Y')
...
_d('com.apple.desktopservices', 'DSDontWriteNetworkStores', '-bool', 'true')
...
_d('com.apple.keychainaccess', 'Show Expired Certificates', '-bool', 'true') if _ask('Keychain shows expired certificates', 'Y')

_d('com.apple.keychainaccess', 'Distinguish Legacy ACLs', '-bool', 'true') if _ask('Makes Keychain Access display *unsigned* ACL entries in italics', 'Y')

# ---------------------------------------------------------------------------
# Remote Desktop

_d('com.apple.remotedesktop', 'IncludeDebugMenu', '-bool', 'true') if _ask('Show the Debug menu in Remote Desktop', 'Y')

_d('com.apple.remotedesktop', 'showShortUserName', '-bool', 'true') if _ask('Define user name display behavior', 'Y')

# ---------------------------------------------------------------------------
# Dock

_d('com.apple.dock', 'tilesize', '-int', '35') if _ask('Set the icon size of Dock items to 35 pixels', 'Y')

_d('com.apple.dock', 'orientation', '-string', 'right') if _ask('Move the dock to the right side of the screen', 'Y')

_d('com.apple.dock', 'minimize-to-application', '-bool', 'true') if _ask("Minimize windows into their application's icon", 'Y')

_d('com.apple.dock', 'static-only', '-bool', 'true') if _ask('Show only active apps in Dock', 'Y')

_d('com.apple.dock', 'mouse-over-hilite-stack', '-bool', 'true') if _ask('Enable highlight hover effect for the grid view of a stack (Dock)', 'Y')

_d('com.apple.dock', 'show-process-indicators', '-bool', 'true') if _ask('Show indicator lights for open applications in the Dock', 'Y')

_d('com.apple.dock', 'launchanim', '-bool', 'true') if _ask('Animate opening applications from the Dock', 'Y')

_d('com.apple.dock', 'mineffect', '-string', 'suck') if _ask('Change minimize/maximize window effect', 'Y')

_d('com.apple.dock', 'expose-animation-duration', '-float', '0.5') if _ask('Speed up Mission Control animations', 'Y')

_d('com.apple.dock', 'notification-always-show-image', '-bool', 'true') if _ask('Show image for notifications', 'Y')

_d('com.apple.dock', 'no-bouncing', '-bool', 'false') if _ask('Enable Bouncing dock icons', 'Y')

_d('com.apple.dock', 'autohide-time-modifier', '-float', '0') if _ask('Remove the animation when hiding or showing the dock', 'Y')

_d('com.apple.dock', 'wvous-show-windows-in-other-spaces', '-bool', 'false') if _ask('In Expose, only show windows from the current space', 'N')

_d('com.apple.dock', 'mru-spaces', '-bool', 'true') if _ask('Automatically rearrange Spaces based on most recent use', 'Y')

_d('com.apple.dock', 'autohide-delay', '-float', '0') if _ask('Remove the auto-hiding Dock delay', 'N')

_d('com.apple.dock', 'autohide', '-bool', 'true') if _ask('Automatically hide and show the Dock', 'Y')

_d('com.apple.dock', 'magnification', '-bool', 'true') if _ask('Automatically magnify the Dock', 'Y')

_d('com.apple.dock', 'showhidden', '-bool', 'true') if _ask('Make Dock icons of hidden applications translucent', 'Y')
...
_d('com.apple.dock', 'showLaunchpadGestureEnabled', '-int', '0') if _ask('Disable the Launchpad gesture (pinch with thumb and three fingers)', 'N')
...
_d('com.apple.Safari', 'ShowFullURLInSmartSearchField', '-bool', 'true') if _ask('Show the full URL in the address bar', 'Y')

_d('com.apple.Safari', 'HomePage', '-string', 'about:blank') if _ask("Set Safari's home page to 'about:blank' for faster loading", 'N')

_d('com.apple.Safari', 'AutoOpenSafeDownloads', '-bool', 'false') if _ask("Prevent Safari from opening 'safe' files automatically after downloading", 'Y')
...
_d('com.apple.Safari', 'ShowSidebarInTopSites', '-bool', 'false') if _ask("Hide Safari's sidebar in Top Sites", 'Y')

_d('com.apple.Safari', 'DebugSnapshotsUpdatePolicy', '-int', '2') if _ask("Disable Safari's thumbnail cache for History and Top Sites", 'Y')

_d('com.apple.Safari', 'IncludeInternalDebugMenu', '-bool', 'true') if _ask("Enable Safari's debug menu", 'Y')

_d('com.apple.Safari', 'FindOnPageMatchesWordStartsOnly', '-bool', 'false') if _ask("Make Safari's search banners default to Contains instead of Starts With", 'Y')

_d('com.apple.Safari', 'ProxiesInBookmarksBar', '()') if _ask("Remove useless icons from Safari's bookmarks bar", 'Y')

_d('com.apple.Safari', 'WarnAboutFraudulentWebsites', '-bool', 'true') if _ask('Warn about fraudulent websites', 'Y')
...
_d('com.apple.Safari', 'SendDoNotTrackHTTPHeader', '-bool', 'true') if _ask("Enable 'Do Not Track'", 'Y')
...
_d('com.apple.Safari', 'IncludeDebugMenu', '-bool', 'true') if _ask("Enable Safari's debug menu", 'Y')

_d('com.apple.safari', 'WebKitDNSPrefetchingEnabled', '-bool', 'true') if _ask('Increase page load speed in Safari (DNS prefetching)', 'Y')

_d('com.apple.Safari', 'WebKitUsesEncodingDetector', '-bool', 'false') if _ask('Disable Data Detectors', 'Y')

_d('com.apple.safari', 'DebugSafari4IncludeGoogleSuggest', '-bool', 'true') if _ask('Google Suggestion', 'Y')

_d('com.apple.safari', 'WebContinuousSpellCheckingEnabled', '-bool', 'true') if _ask('Automatically spell check web forms', 'Y')

_d('com.apple.safari', 'WebGrammarCheckingEnabled', '-bool', 'true') if _ask('Automatically grammar check web forms', 'Y')

_d('com.apple.safari', 'WebKitShouldPrintBackgroundsPreferenceKey', '-bool', 'true') if _ask('Include page background colors and images when printing', 'N')

_d('com.apple.Safari', 'InstallExtensionUpdatesAutomatically', '-bool', 'true')
...
_d('com.apple.mail', 'SpellCheckingBehavior', '-string', 'NoSpellCheckingEnabled') if _ask('Disable automatic spell checking', 'N')

_d('com.apple.mail', 'AddressesIncludeNameOnPasteboard', '-bool', 'false') if _ask("Copy email addresses as 'foo@example.com' instead of 'Foo Bar <foo@example.com>' in Mail.app", 'N')
...
_d('com.apple.mail', 'MinimumHTMLFontSize', '14') if _ask('Set a minimum font size of 14px (affects reading and sending email)', 'N')

_d('com.apple.mail', 'PreferPlainText', '-bool', 'TRUE') if _ask('Force all Mail messages to display as plain text', 'N')

_d('com.apple.mail', 'SuppressAddressHistory', '-bool', 'true') if _ask('Disable tracking of Previous Recipients', 'N')

_d('com.apple.mail', 'SendWindowsFriendlyAttachments', '-bool', 'true') if _ask('Send Windows friendly attachments', 'N')
...
CommandUtils.run_silent('killall', 'mds') if _ask('Load new settings before rebuilding the Spotlight index', 'Y')
...
if _ask('Disable Spotlight keyboard shortcut (Cmd+Space)', 'Y')
  ...
  _pb('Add :AppleSymbolicHotKeys:64:enabled bool false', HOTKEYS_PLIST) unless _pb('Set :AppleSymbolicHotKeys:64:enabled false', HOTKEYS_PLIST) # rubocop:disable Style/SoleNestedConditional
end
...
if _ask('Disable Spotlight Finder search window keyboard shortcut (Cmd+Option+Space)', 'Y')
  ...
  _pb('Add :AppleSymbolicHotKeys:65:enabled bool false', HOTKEYS_PLIST) unless _pb('Set :AppleSymbolicHotKeys:65:enabled false', HOTKEYS_PLIST) # rubocop:disable Style/SoleNestedConditional
end
...
if _ask('Terminal.app settings', 'Y')
  ...
  ['Clear Dark'].each do |profile|
    ...
    # Set-or-Add is atomic (see _pb_set_or_add) for scalar keys; only the array below
    # (noWarnProcesses) still uses Delete-then-Add, which is inherently non-atomic but
    # unavoidable for arrays.
    _pb_set_or_add(TERMINAL_PLIST, ":'Window Settings':'#{profile}':rowCount", '30', 'integer')
    _pb_set_or_add(TERMINAL_PLIST, ":'Window Settings':'#{profile}':columnCount", '120', 'integer')
    ...
    system('osascript', '-e',
           "tell application \"Terminal\" to set font size of settings set \"#{profile}\" to 13")
    ...
    _pb_set_or_add(TERMINAL_PLIST, ":'Window Settings':'#{profile}':useOptionAsMetaKey", 'true', 'bool')
    # Profiles > Shell > "When the shell exits": 0=don't close, 1=close if exited cleanly.
    _pb_set_or_add(TERMINAL_PLIST, ":'Window Settings':'#{profile}':shellExitAction", '1', 'integer')
...
if _ask('iTerm2 settings', 'Y')
  {
    ...
    'DefaultTabBarHeight' => ['-float', '28'],
    'DimBackgroundWindows' => ['-bool', 'true'],
    'DisableTmuxWindowResizing' => ['-bool', 'false'],
    'DisableWindowSizeSnap' => ['-bool', 'false'],
    ...
    'FlashTabBarInFullscreen' => ['-bool', 'true'],
    'HapticFeedbackForEsc' => ['-bool', 'false'],
    'HideTab' => ['-bool', 'false'],
    'IRMemory' => ['-int', '4'],
    # Prefs are stored locally (not in a custom folder or cloud-synced path).
    'LoadPrefsFromCustomFolder' => ['-bool', 'false'],
    'NSAutoFillHeuristicControllerEnabled' => ['-bool', 'false'],
    'NSFontPanelAttributes' => ['-string', '1, 0'],
    'NSNavLastRootDirectory' => ['-string', EnvVars::HOME.join('Desktop').to_s],
    'NSOverlayScrollersFallBackForAccessoryViews' => ['-bool', 'false'],
    'NSQuotedKeystrokeBinding' => ['-string', ''],
    'NSRepeatCountBinding' => ['-string', ''],
    ...
    'NoSyncBrowserUpsell' => ['-bool', 'true'],
    'NoSyncBrowserUpsell_selection' => ['-int', '1'],
    'NoSyncClaudeCodeDiffModeBackfilled' => ['-bool', 'true'],
    'NoSyncClaudeCodeReviewSystemPromptCommandBackfilled' => ['-bool', 'true'],
    'NoSyncClearAllBroadcast' => ['-bool', 'true'],
    'NoSyncClearAllBroadcast_selection' => ['-int', '0'],
    'NoSyncCommandHistoryHasEverBeenUsed' => ['-bool', 'true'],
    'NoSyncConfirmRunOpenFile' => ['-bool', 'true'],
    'NoSyncConfirmRunOpenFile_selection' => ['-int', '0'],
    ...
    'NoSyncEnableAPIServer' => ['-bool', 'true'],
    'NoSyncEnableAPIServer_selection' => ['-int', '0'],
    'NoSyncHaveRequestedFullDiskAccess' => ['-bool', 'true'],
    'NoSyncHaveUsedCopyMode' => ['-bool', 'true'],
    'NoSyncHaveWarnedAboutPasteConfirmationChange' => ['-bool', 'true'],
    'NoSyncIgnoreSystemWindowRestoration' => ['-bool', 'true'],
    'NoSyncKeyCode0MitigationDisabled_Global' => ['-bool', 'true'],
    'NoSyncMigratedDynamicProfileTagToFlag' => ['-bool', 'true'],
    'NoSyncNeverRemindPrefsChangesLostForFile' => ['-bool', 'true'],
    'NoSyncNeverRemindPrefsChangesLostForFile_selection' => ['-int', '1'],
    'NoSyncOnboardingWindowHasBeenShown' => ['-bool', 'true'],
    'NoSyncOnboardingWindowHasBeenShown34' => ['-bool', 'true'],
    'NoSyncOpenLinksInApp' => ['-bool', 'true'],
    'NoSyncOpenLinksInApp_selection' => ['-int', '0'],
    'NoSyncPermissionToShowTip' => ['-bool', 'true'],
    'NoSyncRemoveDeprecatedKeyMappings' => ['-int', '2'],
    'NoSyncRestoreIconAndWindowNameOnHostChange' => ['-bool', 'true'],
    'NoSyncSuppressBadPWDInArrangementWarning' => ['-bool', 'true'],
    'NoSyncSuppressBroadcastInputWarning' => ['-bool', 'true'],
    'NoSyncSuppressBroadcastInputWarning_selection' => ['-bool', 'false'],
    'NoSyncSuppressMissingProfileInArrangementWarning' => ['-bool', 'true'],
    'NoSyncTipsDisabled' => ['-bool', 'true'],
    'NoSyncUserHasSelectedCommand' => ['-bool', 'true'],
    'NoSyncWindowRestoresWorkspaceAtLaunch' => ['-bool', 'false'],
    'NoSyncWorkgroupShortcutsBackfilled' => ['-bool', 'true'],
    'OnlyWhenMoreTabs' => ['-bool', 'false'],
    'OpenArrangementAtStartup' => ['-bool', 'false'],
    'OpenBookmark' => ['-bool', 'false'],
    'OpenNoWindowsAtStartup' => ['-bool', 'false'],
    # 0 = open tmux windows in tabs of the current window.
    'OpenTmuxWindowsIn' => ['-int', '0'],
    'PrefsCustomFolder' => ['-string', ''],
    'PreserveWindowSizeWhenTabBarVisibilityChanges' => ['-bool', 'false'],
    'PreventEscapeSequenceFromClearingHistory' => ['-bool', 'false'],
    'Print In Black And White' => ['-bool', 'true'],
  ...
  _pb("Set :'New Bookmarks':0:'Custom Command' 'No'", ITERM_PLIST)
  ...
  CommandUtils.run_silent(PLISTBUDDY, '-c', "Add :'New Bookmarks' array", ITERM_PLIST)
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:Rows", '48', 'integer')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:Columns", '160', 'integer')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Silence Bell'", 'false', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Unlimited Scrollback'", 'true', 'bool')

  # Profiles > General > Initial directory: 'Recycle' = reuse previous session's directory.
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Custom Directory'", 'Recycle', 'string')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Enable Progress Bars'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Show Status Bar'", 'true', 'bool')

  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Use Cursor Guide'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Visual Bell'", 'true', 'bool')
  ...
  _pb("Add :'New Bookmarks':0:'Jobs to Ignore':6 string zsh", ITERM_PLIST)

  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Minimum Contrast'", '0', 'integer')

  # Profiles > Text
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'ASCII Anti Aliased'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Allow Title Setting'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Ambiguous Double Width'", 'false', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Background Image Location'", "''", 'string')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Blinking Cursor'", 'false', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Blur'", 'false', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Draw Powerline Glyphs'", 'true', 'bool')
  # Horizontal and Vertical Spacing: multipliers relative to font's natural spacing (1.0 = default).
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Horizontal Spacing'", '1', 'real')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Non Ascii Font'", "'Monaco 12'", 'string')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Non-ASCII Anti Aliased'", 'true', 'bool')
  # Keeps background color opaque when transparency > 0; only cursor/text area is affected.
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Only The Default BG Color Uses Transparency'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Transparency'", '0', 'real')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Use Bold Font'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Use Bright Bold'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Use Italic Font'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Use Non-ASCII Font'", 'false', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Vertical Spacing'", '1', 'real')

  # Profiles > Terminal
  # 4 = UTF-8 encoding.
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Character Encoding'", '4', 'integer')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Custom Locale'", "'en_US.UTF-8'", 'string')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Disable Printing'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Idle Code'", '0', 'integer')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Load Shell Integration Automatically'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Mouse Reporting'", 'true', 'bool')
  # 0 scrollback lines with Unlimited Scrollback = true means no hard cap.
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Scrollback Lines'", '0', 'integer')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Send Code When Idle'", 'false', 'bool')
  # 2 = set locale environment variables automatically.
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Set Local Environment Vars'", '2', 'integer')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Terminal Type'", "'xterm-256color'", 'string')

  # Profiles > Window
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Disable Window Resizing'", 'true', 'bool')
  # 1 = use profile name as window/tab icon label.
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:Icon", '1', 'integer')
  # -1 = open new sessions on the screen the window is currently on.
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:Screen", '-1', 'integer')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Sync Title'", 'false', 'bool')
  # 1 = show profile name as the title component.
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Title Components'", '1', 'integer')
  # 0 = normal (non-fullscreen, non-maximized) window type.
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Window Type'", '0', 'integer')

  # Profiles > General / Session
  # BM Growl: post a notification-center alert when bell fires in a background tab.
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'BM Growl'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Close Sessions On End'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:Command", "''", 'string')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Custom Tab Title'", "''", 'string')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Default Bookmark'", "'No'", 'string')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:Description", "'Default'", 'string')
  # Flashing Bell: flash the screen on bell (distinct from Visual Bell which uses a badge).
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Flashing Bell'", 'true', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:Name", "'Default'", 'string')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Open Toolbelt'", 'false', 'bool')
  # 2 = always prompt before closing if a job other than those in 'Jobs to Ignore' is running.
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Prompt Before Closing 2'", '2', 'integer')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:Shortcut", "''", 'string')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Use Custom Tab Title'", 'false', 'bool')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Working Directory'", "'#{EnvVars::HOME}'", 'string')
  ...
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Option Key Sends'", '0', 'integer')
  _pb_set_or_add(ITERM_PLIST, ":'New Bookmarks':0:'Right Option Key Sends'", '0', 'integer')
end
...
if _ask('KeepassXC settings', 'Y')
  _d('org.keepassxc.keepassxc', 'NSNavLastRootDirectory', '-string',
     EnvVars::HOME.join('personal', EnvVars::USER).to_s)
end
...
if _ask('ProtonVpn settings', 'Y')
  ...
  }.each { |key, args| _d('ch.protonvpn.mac', key, *args) }
end
...
if _ask('Zoomus settings', 'Y')
  ...
  }.each { |key, args| _d('us.zoom.xos', key, *args) }
  {
  ...
  }.each { |key, args| _d('ZoomChat', key, *args) }
end
...
if _ask('Clocker settings', 'Y')
  ...
  # defaultPreferences (binary NSData blobs -- not portably expressible).
  {
    ...
    'userFontSize' => ['-int', '7']
  }.each { |key, args| _d('com.abhishek.Clocker', key, *args) }
end
...
if _ask('DBeaver settings', 'Y')
  {
    ...
    'NSScrollAnimationEnabled' => ['-bool', 'false']
  }.each { |key, args| _d('org.jkiss.dbeaver.core.product', key, *args) }
end
...
if _ask('DockDoor settings', 'Y')
  ...
  }.each { |key, args| _d('com.ethanbills.DockDoor', key, *args) }
end
...
if _ask('KeyCastr settings', 'Y')
  ...
  }.each { |key, args| _d('io.github.keycastr', key, *args) }
end
...
if _ask('KeyClu settings', 'Y')
  ...
  }.each { |key, args| _d('com.0804Team.KeyClu', key, *args) }
end
...
if _ask('OnlyOffice settings', 'Y')
  ...
  }.each { |key, args| _d('asc.onlyoffice.ONLYOFFICE', key, *args) }
end
...
if _ask('Shortcat settings', 'Y')
  ...
  # KeyboardShortcuts_* keys are plain JSON strings encoding key codes and modifiers.
  {
    ...
    'rightKeycode' => ['-int', '37'],
    'upKeycode' => ['-int', '40']
  }.each { |key, args| _d('com.sproutcube.Shortcat', key, *args) }
end
...
if _ask('Stats settings', 'Y')
  ...
  }.each { |key, args| _d('eu.exelban.Stats', key, *args) }
end
...
if _ask('Thaw settings', 'Y')
  ...
  }.each { |key, args| _d('com.stonerl.Thaw', key, *args) }
end
...
_d('com.apple.ActivityMonitor', 'OpenMainWindow', '-bool', 'true') if _ask('Show the main window when launching Activity Monitor', 'Y')

_d('com.apple.ActivityMonitor', 'IconType', '-int', '5') if _ask('Visualize CPU usage in the Dock icon', 'Y')

_d('com.apple.ActivityMonitor', 'ShowCategory', '-int', '101') if _ask('Show all processes hierarchically', 'Y')
...
_d('com.apple.ActivityMonitor', 'SelectedTab', '-int', '4') if _ask('Default to showing the Network tab', 'Y')

# ---------------------------------------------------------------------------
# Photos

_dh('com.apple.ImageCapture', 'disableHotPlug', '-bool', 'true') if _ask('Prevent Photos from opening automatically when devices are plugged in', 'Y')

# ---------------------------------------------------------------------------
# Software Update

_d('com.apple.SoftwareUpdate', 'AutomaticCheckEnabled', '-bool', 'true') if _ask('Automatically check for updates (required for any downloads)', 'Y')

_ds('/Library/Preferences/com.apple.SoftwareUpdate', 'AutomaticDownload', '-bool', 'true') if _ask('Download updates automatically in the background', 'Y')

_d('com.apple.commerce', 'AutoUpdate', '-bool', 'true') if _ask('Install app updates automatically', 'Y')

_d('com.apple.commerce', 'AutoUpdateRestartRequired', '-bool', 'true') if _ask('Install macOS updates automatically', 'Y')

_d('com.apple.SoftwareUpdate', 'ConfigDataInstall', '-bool', 'true') if _ask('Install system data file updates automatically', 'Y')

_d('com.apple.SoftwareUpdate', 'CriticalUpdateInstall', '-bool', 'true') if _ask('Install critical security updates automatically', 'Y')

_d('com.apple.SoftwareUpdate', 'ScheduleFrequency', '-int', '1') if _ask('Check for software updates daily, not just once per week', 'Y')

# Mac App Store
_d('com.apple.appstore', 'WebKitDeveloperExtras', '-bool', 'true')
_d('com.apple.appstore', 'ShowDebugMenu', '-bool', 'true')
_d('-g', 'WebKitDeveloperExtras', '-bool', 'true')

# Time Machine
_d('com.apple.TimeMachine', 'DoNotOfferNewDisksForBackup', '-bool', 'true')

# ---------------------------------------------------------------------------
# Screen

# Require password immediately after sleep or screen saver begins.
_d('com.apple.screensaver', 'askForPassword', '-bool', 'true')
_d('com.apple.screensaver', 'askForPasswordDelay', '-int', '0')

# Enable subpixel font rendering on non-Apple LCDs (0=off, 1=light, 2=Medium/flat panel, 3=strong/blurred).
_d('-g', 'AppleFontSmoothing', '-int', '2')
_ds('/Library/Preferences/com.apple.windowserver', 'DisplayResolutionEnabled', '-bool', 'true')

# Screen capture
_d('com.apple.screencapture', 'location', '-string', EnvVars::HOME.join('Desktop').to_s)
_d('com.apple.screencapture', 'type', '-string', 'png')
_d('com.apple.screencapture', 'disable-shadow', '-bool', 'true')
_d('com.apple.screencaptureui', 'thumbnailExpiration', '-float', '15')
...
CommandUtils.run_silent('defaults', 'write', 'com.apple.AddressBook', 'ABBirthDayVisible', '-bool', 'true')
CommandUtils.run_silent('defaults', 'write', 'com.apple.AddressBook', 'ABDefaultAddressCountryCode', '-string', 'in')

# Spaces: when switching applications, switch to respective space.
_d('-g', 'AppleSpacesSwitchOnActivate', '-bool', 'true')

# ---------------------------------------------------------------------------
# Kill affected applications

# Handles core system services (cfprefsd/Dock/Finder/SystemUIServer) and calls
# activateSettings to flush symbolic hotkeys.
...
[
  ...
  'ScreenSaverEngine',
  'SizeUp',
].each { |app| CommandUtils.run_silent('killall', app) }

# Turn off Spotlight indexing for all volumes.
CommandUtils.run_silent('sudo', 'mdutil', '-ai', 'off') if CommandUtils.run_silent('sudo', 'mdutil', '-Eda')
