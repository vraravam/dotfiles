...
OSASCRIPT_CMD = PathUtils::ROOT.join('usr', 'bin', 'osascript').to_s.freeze
...
def _register_legacy(app_name, app_path, hidden)
  items_out, = Open3.capture3(OSASCRIPT_CMD, '-e',
  ...
  system(OSASCRIPT_CMD, '-e', script, out: File::NULL, err: File::NULL)
end
...
app_path_pn = PathUtils::ROOT.join('Applications', "#{app_name}.app")
