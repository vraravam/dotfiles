As documented in the README's [adoption guide](README.md#-getting-started) and [Adoption.md](Adoption.md), this repo and its scripts are aimed at developers/techies. If you are stuck or need help in any fashion, you can reach out to the [owner of the parent repo](https://github.com/vraravam) from where this was forked.
...
* *[files/--HOME--/Brewfile]* Added `cask 'vscodium@insiders'` with a `postinstall:` hook that symlinks the embedded `codium-insiders` binary to `${HOMEBREW_PREFIX}/bin/codium-insiders` -> `codium` -> `code`; commented out the previously-active `cask 'zed@preview'` entry (and its own postinstall symlink) now that VSCodium is the default editor again. Uncommented the previously-dormant VSCode extensions block (30 extensions -- `shan.code-settings-sync` intentionally excluded, see below -- still guarded on `code` being in `PATH`). Set the `VSCODE_GALLERY_*` marketplace-redirect env vars directly via `ENV[...] =` in the Brewfile itself (not just relying on the equivalent `.aliases` block) -- on a vanilla-OS fresh install, the full Brewfile (including this cask and its extensions) runs as a detached background job forked before `.aliases` is ever re-sourced with `codium` now present, so the shell-level redirect alone would never be active for extensions installed this way; setting it in the Brewfile guarantees it applies regardless of which shell/background context invoked `brew bundle`.
* *[files/--ZDOTDIR--/.aliases]* Reordered `preferred_editors` to try `code --wait` before `zed --wait`.
* *[files/--HOME--/custom.gitignore]* Replaced the old blanket `/.vscod*/` directory-ignore with `/.vscod*/*` plus explicit negations for `argv.json`/`settings.json`/`tasks.json`/`launch.json`/`extensions.json`/`*.code-snippets` -- the old trailing-slash form silently defeated `~/.gitignore_global`'s own per-workspace `.vscode/*` negation pattern (git's rule: a directory matched by a trailing-slash ignore pattern is never descended into to check later negations, from any source, once matched). The new pattern also uniformly covers `.vscode`, `.vscode-insiders`, and `.vscode-oss-shared` (previously handled by its own now-redundant separate line, removed); `argv.json` is tracked since it's the only genuinely hand-editable file in a global install dir (its `crash-reporter-id` is a machine-specific, harmless-to-duplicate value).
* *[files/--HOME--/Library/Application Support/VSCodium - Insiders/User/settings.json, keybindings.json]* Adopted via `install-dotfiles.rb`'s existing generic `--VAR--` symlink mechanism -- no code changes needed, since `--HOME--` already resolves and `Pathname#mkpath` recursively creates the full missing parent directory chain even before VSCodium itself is ever installed on a vanilla OS. Deliberately excluded from tracking: `extensions/`/`cli/` (regenerated by the Brewfile's `vscode` DSL entries), `globalStorage/`/`workspaceStorage/`/`History/` (extension-managed state/cache), and `syncLocalSettings.json` (contained a live GitHub personal access token belonging to the now-removed `shan.code-settings-sync` extension).
* *[files/--HOME--/Brewfile, files/--HOME--/Library/Application Support/VSCodium - Insiders/User/settings.json]* Removed the `shan.code-settings-sync` extension and its `sync.gist`/`sync.forceDownload`/`sync.quietSync`/`sync.autoDownload`/`sync.autoUpload` settings keys -- fully redundant now that `settings.json`/`keybindings.json` are git-tracked directly and extensions are Brewfile-declared, and the referenced Gist ID was a genuine exposure risk in this public repo (GitHub "secret" gists rely on URL obscurity rather than real access control, so publishing the ID alone lets anyone construct the direct link).
* *[files/--XDG_CONFIG_HOME--/git/config-delta.inc]* Changed `hyperlinks-file-link-format` from `zed://file/{path}:{line}` to `vscodium-insiders://file/{path}:{line}` -- confirmed the correct scheme by inspecting the installed app's `Info.plist` (`CFBundleURLSchemes`), since VS Code, VSCodium, and VSCodium Insiders each register a distinct scheme name (`vscode`, `vscodium`, `vscodium-insiders` respectively) rather than sharing one.
* *[scripts/wait-editor]* Updated the `'zed --wait'` example references in comments to `'code --wait'`. Added a comment explaining why this script must remain a standalone executable file and can never be converted to a shell function: `crontab -e`/`visudo`/`fc` invoke `$EDITOR` from separate processes that either `execvp()` it directly or fork a fresh subshell, neither of which has any visibility into an interactive shell's function table.

#### Adopting these changes

* Run `ruby scripts/install-dotfiles.rb` to symlink the newly-tracked VSCodium `settings.json`/`keybindings.json` into place.
* Run `brew bundle install` to install `vscodium@insiders` and its extensions list.
* Restart Terminal/iTerm to pick up the reordered `preferred_editors` in `.aliases`.
...
* *[scripts/utilities/encrypted_backup.rb, scripts/git-remote-encrypted-backup, scripts/setup-encrypted-backup.rb]* Deleted -- this repo's own implementation of the gpg+git-bundle encrypted backup has been extracted into a standalone, general-purpose tool, [`git-remote-gpg-encrypt`](https://github.com/vraravam/git-remote-gpg-encrypt), fully decoupled from this repo's `Logging`/`EnvVars`/`GitProcessor`/`CliParser` infrastructure so it can be used outside of dotfiles too. It's distributed via a personal Homebrew tap, [`homebrew-tap`](https://github.com/vraravam/homebrew-tap) (deliberately generic-named, not tied to this one tool -- intended to hold formulae for future personal tools too).
* *[files/--HOME--/Brewfile]* Removed the standalone `brew 'gnupg'` line; added `brew 'vraravam/tap/git-remote-gpg-encrypt', trusted: true` (a fully-qualified formula reference, which auto-taps `vraravam/tap` -- no separate `tap` line, matching this Brewfile's existing convention for other third-party taps, e.g. `anomalyco/tap/opencode`) gated behind a new `encrypted_backup_enabled` check -- mirrors the existing `keybase_enabled` pattern (reads `.shellrc` directly rather than `ENV`, since `brew bundle` does not inherit arbitrary shell exports) -- so the formula only installs when `ENCRYPTED_HOME_REPO_URL` or `ENCRYPTED_PROFILES_REPO_URL` is exported in `.shellrc`. The formula pulls in both `gnupg` and `git` transitively, so nothing else needs to change to keep them available during vanilla-OS bootstrap.
* *[files/--HOME--/.shellrc, scripts/utilities/env_vars.rb]* Renamed `ENCRYPTED_HOME_REPO_NAME`/`ENCRYPTED_PROFILES_REPO_NAME` to `ENCRYPTED_HOME_REPO_URL`/`ENCRYPTED_PROFILES_REPO_URL` -- the external tool's `gpg-encrypt::` remote scheme takes a full git URL (any host), not a bare repo name with an implicitly-derived GitHub owner the way this repo's own embedded implementation used to. `KEYBASE_HOME_REPO_NAME`/`KEYBASE_PROFILES_REPO_NAME` are unaffected (Keybase's own `keybase://` scheme is unchanged).
* *[scripts/fresh-install-of-osx.sh]* Renamed `ENCRYPTED_*_REPO_NAME` references to `ENCRYPTED_*_REPO_URL` throughout (env var checks, log/skip messages). The "Setup encrypted backup" step now calls `git-gpg-encrypt-setup` instead of `setup-encrypted-backup.rb`. The early Keychain-passphrase reminder (printed right after `.shellrc` is sourced) now references the new default Keychain service name, `git-remote-gpg-encrypt` (was `gpg-encrypted-backup`). See the "Route `git-gpg-encrypt-restore`..." entry below for how `_clone_home_repo`/`_clone_profiles_repo` actually perform the clone.
* *[scripts/recreate-repository.rb]* Removed `_encrypted_backup_repo_name`/the force-mode auto-detection of encrypted-backup wrapper-repo directories and the automatic `EncryptedBackup.verify_current_blob_decryptable?` safety check it triggered -- the external tool's wrapper-repo cache directory is now keyed by an opaque SHA256 hash of the remote URL (an internal implementation detail this repo has no business reaching into, unlike the old bare-name-based cache path this repo could previously reconstruct). Run `git gpg-encrypt-verify <url>` yourself before squashing/force-pushing a wrapper-repo directory if you need that safety check now.
* *[scripts/utilities/git_processor.rb, files/--HOME--/.shellrc]* Removed the comment's reference to the now-deleted `encrypted_backup.rb` module (see the "Route `git-gpg-encrypt-restore`..." entry below for how `clone_repo_into` relates to the external tool today). `SKIP_POST_CLONE_MAINTENANCE`'s doc comment no longer cites the now-deleted `EncryptedBackup._ensure_wrapper_repo` as its motivating example (no current caller passes `skip_maintenance: true`; the mechanism is kept for any future ephemeral-clone use case with the same shape).
* *[KeybaseMigration.md]* Rewritten to describe the external tool's workflow: `git gpg-encrypt-setup`/`-verify`/`-restore` instead of the old Ruby-module/script names, `gpg-encrypt::<full-url>` instead of `encrypted-backup::<name>`, and the new Keychain service name. The detailed design rationale (chunking mechanics, remote-helper protocol, KDF comparison against Keybase) now points at the external repo's own `README.md`/`docs/DESIGN.md` instead of duplicating it here.
* *[TechnicalDeepDive.md]* Section 14 trimmed to a summary of the dotfiles-side integration points (env vars, Brewfile tap/formula, fresh-install call sites) plus a pointer to the external tool's own docs -- the four-way tool comparison and git-remote-gcrypt rejection history previously duplicated here now live authoritatively in that repo (which also evaluates two additional alternatives, `git-remote-sealed` and `age`, discovered after this repo's version was written).
* *[Extras.md]* Removed the `setup-encrypted-backup.rb` entry (script no longer exists in this repo); updated `recreate-repository.rb`'s entry to describe the new manual (`git gpg-encrypt-verify`) verification step instead of the removed automatic wrapper-repo detection.
* *[Adoption.md]* Updated § 1.4 and § 2.3.C's encrypted-backup instructions to the new env var names, `gpg-encrypt::<full-url>` remote syntax, `git gpg-encrypt-setup` command, and `git-remote-gpg-encrypt` Keychain service name.
* *[AGENTS.md]* Added `git-remote-gpg-encrypt` and `homebrew-tap` as known related repos (`${PROJECTS_BASE_DIR}/oss/`) -- standalone, not managed by `install-dotfiles.rb`, with their own git history and (for the former) its own adapted copy of this repo's Ruby conventions. Documents the backport relationship: a Ruby-rule change made here may need an equivalent, adapted version applied there too.
* *[.ai/domains/ruby-scripting.md]* Removed `encrypted_backup.rb` from the example list of modules using the `extend self` pattern (file no longer exists).

#### Route `git-gpg-encrypt-restore` through `clone_repo_into`'s unified origin/branch handling instead of duplicating it in fresh-install

* *[files/--HOME--/.shellrc]* `clone_repo_into` now accepts a third, mutually-exclusive `gpg-encrypt::<clone-url>` scheme for its first argument, alongside a plain clone URL and the existing `keybase://` special-case. When given, it invokes `git-gpg-encrypt-restore <url> <target-folder>` directly (bypassing git's own subcommand dispatch, which would otherwise intercept `-h`/`--help`) instead of the caller doing so itself. Origin-remote cleanup (stripping the scratch-path `origin` that a bundle-based restore leaves behind) and branch tracking setup are now unified across both the pre-existing bundle-import path and this new path, instead of being duplicated ad hoc by each caller. Branch enforcement (`target_branch` argument) is likewise now unified across all three paths (network clone, bundle-import, `gpg-encrypt::`) -- previously it only applied to a plain network clone, so a bundle-import or `gpg-encrypt::` restore landing on a branch other than the one requested went silently unenforced; it now falls back to `git switch <target_branch>` (relying on git's normal DWIM remote-tracking-branch lookup) before erroring out. Added upfront validation of the first argument's URL scheme (`https://`, `http://`, `ssh://`, `git://`, `git+ssh://`, `file://`, `keybase://`, `gpg-encrypt::`, or scp-style `user@host:path`), erroring out immediately on anything unrecognized instead of only failing later, deep inside whichever path happened to be taken.
* *[scripts/fresh-install-of-osx.sh]* `_clone_home_repo`/`_clone_profiles_repo` now call `clone_repo_into "gpg-encrypt::${encrypted_repo_url}" ...` instead of calling `git-gpg-encrypt-restore` directly followed by manually adding an `origin`/`origin2` remote -- that remote-naming logic (claim `origin2` instead of `origin` when Keybase is also configured, so `_configure_backup_remote` can still claim `origin` for it) was duplicated across both functions and is now handled once, uniformly, inside `clone_repo_into` itself.
* *[scripts/utilities/git_processor.rb]* Updated the comment above `GitProcessor.clone_repo_into` and its `@param url` doc to describe the new `gpg-encrypt::<url>` special-case in the shell function it delegates to (no Ruby caller uses it yet -- `resurrect-repositories.rb` remains its only consumer, via the pre-existing bundle-import path).

#### Regenerate `.gitignore` from gitignore.io templates; add LSP cache directories to the global gitignore

* *[.gitignore]* Regenerated from `https://www.toptal.com/developers/gitignore/api/vim,emacs,visualstudiocode,sublimetext,jetbrains,macos,linux,windows,ruby`, replacing a handful of hand-maintained entries (`/vendor/`, `/.bundle/`, `/coverage/`) that the generated Ruby template already covers. The project-specific patterns (`*.zwc*`, `/.ruby-lsp/*`, `/.ai/session-state/`, the forced `.envrc`/`lib/` includes) remain below the generated block, unchanged.
* *[files/--HOME--/.gitignore_global]* Added LSP cache/index directories for several language servers not previously covered: `.ruby-lsp/`, `.metals/`, `.bloop/`, `project/**/metals.sbt`, `.elixir_ls/`, `.ccls-cache/` -- each named after a specific language server, so there's no realistic collision risk with an actual project source directory, unlike the generic `bin/`/`lib/` patterns already present elsewhere in this file.

#### Adopting these changes

* Run `brew bundle install` to install the new tap/formula (also removes the direct `gnupg` dependency line, though the package itself stays installed transitively) -- this is now a no-op if you already have `ENCRYPTED_HOME_REPO_URL`/`ENCRYPTED_PROFILES_REPO_URL` set (the common case), but machines with neither set will no longer install `git-remote-gpg-encrypt` automatically.
* Update `.shellrc`: rename `ENCRYPTED_HOME_REPO_NAME='home'`/`ENCRYPTED_PROFILES_REPO_NAME='browser-profiles'` to `ENCRYPTED_HOME_REPO_URL='https://github.com/<you>/home.git'`/`ENCRYPTED_PROFILES_REPO_URL='https://github.com/<you>/browser-profiles.git'` (or your own git host).
* Reconfigure any repo currently using an `encrypted-backup::<name>` remote to the new `gpg-encrypt::<full-url>` scheme: `git remote set-url <name> gpg-encrypt::<full-url>`.
* One-time-per-machine: run `git gpg-encrypt-setup` (replaces `setup-encrypted-backup.rb`). Note the Keychain service name changed (old: `gpg-encrypted-backup`, new: `git-remote-gpg-encrypt`) -- the passphrase will need to be re-entered once per machine: `security add-generic-password -A -a "${USER}" -s 'git-remote-gpg-encrypt' -w`.
* Restart Terminal/iTerm to reload `.shellrc`'s renamed env vars and updated `clone_repo_into` function.
* If you re-run `fresh-install-of-osx.sh` to pick up its `_clone_home_repo`/`_clone_profiles_repo` changes, it is idempotent and safe to run again on an already-configured machine.
...
#### Eliminate GH_USERNAME/Keybase pre-configuration, add CI and test coverage, restore basic/advanced docs, remove stale historical narrative from `.ai/`

* *[files/--HOME--/.shellrc, scripts/utilities/env_vars.rb, scripts/fresh-install-of-osx.sh]* `GH_USERNAME` no longer needs to be stored anywhere in a fork. It's typed once into the bootstrap command; `_resolve_gh_username()` derives it automatically on every later run by reading `${DOTFILES_DIR}`'s `origin` remote. Removed the now-dead `EnvVars::GH_USERNAME`/`EnvVars::UPSTREAM_GH_USERNAME` constants (never actually read anywhere). Simplified the upstream-remote setup in `fresh-install-of-osx.sh` to always call `add-upstream-git-config.rb` -- it already no-ops correctly when origin's owner matches, so the separate `GH_USERNAME != UPSTREAM_GH_USERNAME` guard was redundant.
* *[scripts/utilities/keybase.rb, files/--HOME--/.shellrc, scripts/utilities/env_vars.rb, scripts/fresh-install-of-osx.sh, files/--HOME--/Brewfile]* Keybase requires zero pre-configuration now. `Keybase.username`/`ensure_logged_in` derive identity live from `keybase status --json` instead of a stored `KEYBASE_USERNAME` (which previously defaulted to the original author's own account -- a real risk for a fresh fork that edited nothing). Removed `KEYBASE_USERNAME`/`KEYBASE_HOME_REPO_NAME`/`KEYBASE_PROFILES_REPO_NAME` from `.shellrc` and `env_vars.rb` entirely. `fresh-install-of-osx.sh` now installs the Keybase cask unconditionally (same as any other package) and gates actual use on a one-time interactive prompt -- asked only on `FIRST_INSTALL` and only if not already logged in, reading from `/dev/tty` explicitly (since the script runs via `curl | zsh`, plain `read` would read from the script's own stdin instead of the terminal). Every later run is silent: syncs automatically if logged in, skips silently otherwise. Fixed a real bug in the process: a failed/declined Keybase login previously hard-aborted the entire fresh-install (`|| return 1`); it's now a non-fatal warning.
* *[Gemfile]* Added a `ruby '2.6.10'` version pin so Bundler (and Dependabot, if configured later) can never resolve a gem requiring a newer Ruby than this repo's scripts actually support.
* *[.github/workflows/lint.yml, .github/workflows/rspec.yml, .github/workflows/system-ruby-version-check.yml, .github/scripts/check-system-ruby-version.sh, Gemfile, Gemfile.lock, .gitignore]* Added CI: `lint.yml` runs `zsh -n`, `ruby -c`, and `rubocop` on every push/PR to any branch (skipping doc-only changes); `rspec.yml` runs the new test suite the same way; `system-ruby-version-check.yml` runs weekly (plus manual dispatch) to compare the macOS system Ruby against the `Gemfile` pin and files a GitHub issue if Apple/GitHub's runner image ever drifts, since that drift would otherwise only surface the next time someone happens to push. `rubocop` (0.93.1) and `rspec` (3.13.2) are pinned in `Gemfile`/`Gemfile.lock` to versions that install cleanly under Ruby 2.6 (newer releases of both depend on the `prism` parser gem, which requires Ruby >= 2.7).
* *[spec/]* Added an RSpec suite (`spec_helper.rb`, `.rspec`, 7 spec files, 116 examples) covering `Core`, `String`/`Hash`/`Enumerable` extensions, `EnvVars`'s runtime-flag methods, `GitProcessor::GitUrlParser`, and the newly-redesigned `Keybase` module (all external commands mocked -- no real `keybase`/`git` dependency to run the suite).
* *[.github/dependabot.yml, Gemfile, .github/workflows/dependabot-audit.yml, .github/instructions/dependabot-security-review.instructions.md, .github/agents/README.md]* Added automated dependency-update review: `dependabot.yml` enables weekly `bundler` version updates with an explicit 3-day cooldown (matches Dependabot's own default, made explicit here for visibility). `bundler-audit` (pinned to `0.9.3`, verified installable under Ruby 2.6) is added to `Gemfile` and run against every Dependabot PR's updated `Gemfile.lock` by a new `dependabot-audit.yml` workflow, which fails the check on any known CVE and requests a GitHub Copilot code review (`gh pr edit --add-reviewer @copilot`) on every such PR. `dependabot-security-review.instructions.md` is a path-specific Copilot instructions file (`applyTo: Gemfile,Gemfile.lock`) that narrows `.github/agents/security-reviewer.agent.md`'s general script-security checklist down to what's actually relevant for a dependency bump (known CVEs, the Ruby 2.6 ceiling, unexpected transitive dependencies, gem source/provenance, pin-vs-range changes) -- Copilot reads it natively for any PR touching those two files, no manual prompting required. `.github/agents/README.md` documents this as the one automated exception to the "these agents are manual-invocation-only" rule that governs the other three agent files.
* *[README.md, Adoption.md, Advanced.md]* Restored the basic/advanced documentation split: `README.md` trimmed from 141 to ~50 lines; `Adoption.md` now covers only the basic path (Phases 1-3: prepare, fork/customize, first-time setup), with Phase 2's required-customization step reduced to the one unavoidable value (`GH_USERNAME`, typed into the bootstrap command -- no file edit, no GitHub-UI squash dance required for a plain fork); Phases 4-5 (ongoing maintenance, staying up-to-date) moved to a new `Advanced.md`. Phase 3.1's pre-flight checklist now explicitly separates hard requirements (FileVault, Full Disk Access -- the script aborts without them) from conditional (`mas` login) and soft-recommended (Accessibility permission) items, instead of labeling all of them "recommended."
* *[Extras.md]* Rewrote the "Git hook customizations" section -- it previously documented a `post-push-<repo>.sh` hook as if it worked, directly contradicting `Advanced.md`/`.ai/domains/git-config.md` ("git has no `post-push` hook, this is intentional upstream design"). Now points to `Advanced.md`'s wrapper-function pattern instead of duplicating (and contradicting) it.
* *[CONTRIBUTING.md]* Fixed every link in the Table of Contents (9 of 9 were broken) -- every heading is emoji-prefixed, so the real GitHub anchor needs a leading hyphen (`#-getting-started`, not `#getting-started`) that the TOC omitted.
* *[README.md, TechnicalDeepDive.md, AGENTS.md, .ai/context.md]* Corrected a stale shell-startup performance claim. Verified empirically (20-run average, `time zsh -i -c exit`, Apple M3 Pro): actual current startup is ~30ms, matching the original README claim -- `.ai/context.md`'s "78-87ms average" figure (which several other docs had started citing as "current") was itself a stale June 2026 snapshot, superseded by later optimization work and never updated.
* *[Adoption.md, Extras.md, TechnicalDeepDive.md]* Fixed several broken cross-file links: `Adoption.md`'s link to a README section deleted during the restructuring above; two double-hyphen anchor mismatches in `Extras.md` pointing at `TechnicalDeepDive.md`; `TechnicalDeepDive.md`'s own Table of Contents self-reference for its `.shellrc`/`.aliases` architecture section.
* *[Adoption.md, Extras.md]* Reduced incidental references to the maintainer's personal GitHub/Keybase usernames in illustrative examples (replaced with generic placeholders), while keeping the handful of references that are functionally required (e.g. the actual upstream repo URL to fork from, and the bootstrap command that must show a real, working default for copy-paste use).
* *[.ai/context.md, .ai/domains/ruby-scripting.md, .ai/README.md, .github/agents/README.md]* Removed stale historical narrative that no longer serves future work: `.ai/context.md` shrank from a 505-line "session journal" (dated bug-fix write-ups, optimization milestones, a duplicated copy of rules that already live in `.ai/domains/`) to 173 lines of durable navigation/reference material, since every rule it restated is already the authoritative version in the relevant `domains/` file. Removed a 76-line dead "Legacy Single-Mode" Ruby script template from `ruby-scripting.md` (superseded by the mandatory dual-mode pattern, kept "for reference" only). Updated stale "historical insights" framing of `.ai/context.md` in `.ai/README.md` and `.github/agents/README.md` to match its new purpose. Kept the handful of historical notes that inform genuinely future-relevant work (e.g. the Antidote `.zwc` compilation constraint, the Ruby-migration rebase/refactoring methodology).
* *[.ai/REBASE-AND-REFACTORING-METHODOLOGY.md, .ai/instructions.md]* Added two new generalized workflow sections: "Forward Rebase: Catching Up a Branch Chain from Its Parent" (bringing a WIP conversion branch up to date with its own parent branch, not necessarily `master`, including reusing new parent-branch utilities and stripping file-extension-conversion noise from docs/comments) and "Backporting: Bringing Branch Improvements Back to Master" (landing generally-useful improvements from WIP branches onto `master` without merging their incomplete conversion work). Both codify rules from previously-external prompt files so they no longer need to be kept outside the repo.
* *[scripts/software-updates-cron.rb]* Added an hourly `zsh-patina restart` cron step -- the daemon can die after OS sleep/wake or a crash, and the existing `.zshrc` startup check only ever notices on the *next new* shell; this covers the gap for already-open terminal sessions.
* *[files/--ZDOTDIR--/.zshrc]* Simplified the `_check_patina_restart` function from a ~65-line `pgrep`/`ps`/`zstat` mtime-comparison dance down to an unconditional, idempotent `zsh-patina restart` call -- `zsh-patina restart` already handles both "not running" and "config changed" cases internally, so the extra bookkeeping added no value.
* *[CHANGELOG.md]* Fixed the intro line's link to the README (anchor changed when README was restructured above).

#### Adopting these changes

* Restart Terminal/iTerm to pick up the `.shellrc`/`.zshrc` changes (removed env vars, simplified `zsh-patina` check).
* If you're re-running the bootstrap command on a machine that already has this repo cloned, you no longer need `GH_USERNAME` in the command at all -- it's derived automatically. It's still required for a genuinely new/vanilla machine.
* If you use Keybase, nothing to do -- your existing login is picked up automatically. If you don't, nothing changes either; you'll just see a new one-time prompt on your next `FIRST_INSTALL` run (never on ordinary re-runs).
* Run `bundle install` once (see the new `Gemfile`) if you want to run `rubocop`/`rspec`/`bundler-audit` locally -- CI does this automatically.
* If you enable Dependabot for your fork, its `bundler` PRs will now be audited and Copilot-reviewed automatically; no setup needed beyond having Dependabot enabled and (optionally) Copilot code review enabled for the repository.
...
#### Add safer force-push alias and reflog-based push safety check

* *[files/--XDG_CONFIG_HOME--/git/config]* Added `pushf` alias (`push --force-with-lease`) as a safer, easy-to-reach-for alternative to `push --force`. Added `push.useForceIfIncludes = true`, which strengthens `--force-with-lease` pushes by also verifying via reflog that the remote-tracking tip is an ancestor of what's being pushed, not just that the remote currently matches it -- has no effect on plain `--force`. Note: relies on local reflog history, which `gc.reflogexpire`/`gc.reflogexpireunreachable` (already set to `2.weeks.ago` in this repo) can age out, occasionally requiring `--no-force-if-includes` as a fallback for an otherwise-legitimate force-push.
...
* *[files/--HOME--/.shellrc]* Added a zero-fork `export PATH="${HOMEBREW_PREFIX}/bin:${HOMEBREW_PREFIX}/sbin:${PATH}"` immediately after `HOMEBREW_PREFIX` is determined. Previously `.shellrc` only exported `HOMEBREW_PREFIX` without ever prepending it to `PATH` -- the richer `brew shellenv` caching that actually reordered `PATH` lived only in `.zshrc`, which only runs for interactive shells. Any non-interactive context that sources `.shellrc` directly (cron jobs, scripts, direnv's bash subshell) inherited whatever base PATH it started with, `/usr/bin` first, exactly matching `brew doctor`'s "`/usr/bin occurs before /opt/homebrew/bin in your PATH`" warning. Interactive shells were unaffected by this gap and already ordered correctly via `.zshrc`'s existing `brew-shellenv-cache.zsh` mechanism; this addition is harmless there too since the final `typeset -gU ... PATH path ...` in `.zshrc` dedupes the result.
* *[files/--HOME--/.shellrc]* `load_file_if_exists` now calls `recompile_zsh_script` on the target file before sourcing it, keeping the `.zwc` bytecode cache in sync in every context that sources a file this way -- not just the interactive shells covered by `.zlogin`'s bulk recompile pass. Measured overhead is negligible (~0.05ms per call, no fork; only actually recompiles when stale).
* *[files/--HOME--/.shellrc]* Fixed a latent bug in `recompile_zsh_script` this change exposed: `zrecompile` was only ever `autoload`ed by `.zshrc` (interactive shells), so any caller reaching `recompile_zsh_script` without `.zshrc` having run first (cron jobs, scripts, the `load_file_if_exists` safeguard above from a non-interactive context) called an undefined function that was silently swallowed by the existing `|| true`, appearing to succeed while never actually writing a fresh `.zwc`. Now guards `autoload -Uz zrecompile` behind `is_zsh` (idempotent, no fork) right before the recompile attempt.
* *[files/--HOME--/.shellrc]* Fixed a stale comment on `load_file_if_exists`'s `source` call describing a `|| true` pattern that hasn't existed since the line was changed to `|| warn "..."` in an earlier commit.

#### Fix widespread bash-compatibility bugs found via a full `.shellrc` POSIX/bash audit

Prompted by the `load_file_if_exists` work above, did a systematic audit of every zsh-specific construct in `.shellrc` for correctness under bash (direnv's `.envrc` sources this file in a bash subshell, and any script may `source ~/.shellrc` directly from bash). Found and fixed several bugs where bash silently produced wrong results with no error at all, plus a few where bash failed to even parse the file. See `.ai/domains/shell-scripting.md`'s new "Bash Compatibility Gotchas Discovered in `.shellrc`" section for the full catalog with confirmed repro commands for each.

* *[files/--HOME--/.shellrc]* `_log_indent`: `[[ -v "arr[key]" ]]` broke bash's own `[[ ]]` parser at *definition time* -- confirmed this is not fixable with an `is_zsh` guard (bash must tokenize a function's body to find its closing brace when defining it, even if never called), so the entire file failed to source under bash. Replaced with `[[ -z "${arr[key]:-}" ]]` (emptiness check instead of existence check).
...
Verification: `zsh -n`/`bash -n` clean, clean `source` under both shells (exit 0), and a full functional regression (8 affected functions) producing identical output in both shells.

#### Adopting these changes

* **Recommended:** Quit and restart Terminal/iTerm2 entirely (not just open a new window/tab). A new window is sufficient on its own -- it's a completely fresh shell process, so `is_shellrc_sourced` isn't defined yet and `.shellrc` runs from scratch -- but leaving old windows open risks switching back to one and assuming the fix is live when that session already sourced the old `.shellrc` before this change existed.
* To fix an already-open session in place instead of restarting: run `unfunction is_shellrc_sourced; load_file_if_exists ~/.shellrc`. Removing `is_shellrc_sourced` clears the guard that would otherwise make a plain re-source a no-op; `load_file_if_exists` then recompiles the `.zwc` (via the `recompile_zsh_script` fix above) and re-sources `.shellrc` in one step.
...
* *[scripts/utilities/core.rb]* Added `current_timestamp()` method returning `'YYYY-MM-DD HH:MM:SS'` format - single source of truth for all timestamp formatting across Ruby codebase. Added `running_in_tty?()` method checking if stdout is TTY or `FORCE_COLOR` set - gates interactive operations (app kill/restart, prompts) that should not run in cron/non-interactive contexts. Added comprehensive 35-line documentation defining strict rules for Core module: ZERO requires except Ruby stdlib, OS-agnostic only (no macOS/Linux-specific calls), check ENV directly (not via EnvVars to avoid circular dependency), with examples of appropriate vs inappropriate methods. These methods moved from MacOS module to eliminate circular dependency (macos.rb required logging.rb which used MacOS.current_timestamp).

* *[scripts/utilities/macos.rb]* Removed `current_timestamp()` and `running_in_tty?()` methods (now in Core module). Updated `LOGIN_ITEM_APPS` comment to document this is single source of truth (shell array `_MACOS_LOGIN_ITEM_APPS` in .aliases was dead code, removed). Added `sleep 1` after killing apps in `kill_login_item_apps()` to ensure full termination before defaults writes begin - prevents race where app's shutdown handler might flush preferences after we've started writing new values. Updated documentation explaining the sleep prevents race conditions.

* *[scripts/utilities/git_processor.rb]* Enhanced `smart_commit()` method to auto-generate commit messages when called without arguments. Detects repository state via new `commit_count()` helper - returns 0 for brand new repos, >0 for repos with history. Auto-generates "Initial commit: <timestamp>" for first commit (count == 0) or "Incremental commit: <timestamp>" otherwise. Changed signature from `smart_commit(message)` to `smart_commit(message = nil)` to support optional message. Uses `Core.current_timestamp` for consistent formatting. Works correctly for brand new repos, repos without remotes, and after `git.recreate()`. Removed `require_relative 'macos'` from requires block - now uses `Core.current_timestamp` instead of `MacOS.current_timestamp`.

* *[scripts/utilities/logging.rb]* Updated `print_script_start()` and `print_script_duration()` to use `Core.current_timestamp` instead of inline `Time.now.strftime('%Y-%m-%d %H:%M:%S')`. Eliminated circular dependency: logging.rb previously required macos.rb for timestamps, which itself required logging.rb for log methods. Core module has zero dependencies, breaking the cycle.

* *[scripts/utilities/git_workspace.rb]* Simplified `_commit_repo()` to use `git.smart_commit` with no arguments - auto-generates "Incremental commit: <timestamp>" message instead of manually constructing timestamp string. Removed manual `Time.now.strftime` call and `timestamp` variable.

* *[scripts/software-updates-cron.rb]* Updated 3 call sites to use `Core.current_timestamp` instead of `MacOS.current_timestamp`. Added `require_relative 'utilities/core'` to requires block. Maintains consistent timestamp format across all log entries and status updates.

* *[scripts/capture-prefs.rb]* Updated to use `Core.running_in_tty?` instead of `MacOS.running_in_tty?` in conditional for killing/restarting login-item apps. Only kills apps on import when running interactively (not in cron). Added `require_relative 'utilities/core'` to requires block.

* *[scripts/recreate-repository.rb]* Simplified to use `git.smart_commit` with no arguments instead of manually constructing commit message with `MacOS.current_timestamp` and force/incremental prefix logic. Auto-generation in `smart_commit` handles Initial vs Incremental detection via commit count, eliminating duplicate logic.

* *[scripts/osx-defaults.sh]* Updated comment documenting canonical login-item app list location changed from `_MACOS_LOGIN_ITEM_APPS` (.aliases § 3n) to `MacOS::LOGIN_ITEM_APPS` (scripts/utilities/macos.rb). Shell array was dead code (never referenced, only Ruby constant used).

* *[files/--HOME--/.aliases]* Removed 26-line dead code `_MACOS_LOGIN_ITEM_APPS` array. This shell array was never used - all scripts call Ruby `MacOS.kill_login_item_apps` and `MacOS.restart_login_item_apps` which read from `MacOS::LOGIN_ITEM_APPS` constant. Comment claimed array was duplicated in both .aliases and macos.rb, but shell version was never referenced. Ruby constant is single source of truth.

#### Adopting these changes

* No user action required - changes are internal refactoring
* Timestamp format unchanged: all timestamps still display as `YYYY-MM-DD HH:MM:SS`
* Git commit messages auto-generated by `smart_commit` use same format as before
* Login-item apps behavior unchanged (still killed/restarted during defaults writes)
