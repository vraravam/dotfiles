...
* *[files/--HOME--/Brewfile]* Removed the standalone `brew 'gnupg'` line; added `brew 'vraravam/tap/git-remote-gpg-encrypt', trusted: true` in the same base (pre-`FIRST_INSTALL`) section -- a fully-qualified formula reference auto-taps `vraravam/tap` (no separate `tap` line, matching this Brewfile's existing convention for other third-party taps, e.g. `anomalyco/tap/opencode`). The formula pulls in both `gnupg` and `git` transitively, so nothing else needs to change to keep them available during vanilla-OS bootstrap.
...
* *[scripts/fresh-install-of-osx.sh]* `_clone_home_repo`/`_clone_profiles_repo` now call `git-gpg-encrypt-restore <url> <target-dir>` instead of `EncryptedBackup.clone_and_decrypt` (Ruby), and configure `gpg-encrypt::<url>` remotes instead of `encrypted-backup::<name>`. The "Setup encrypted backup" step now calls `git-gpg-encrypt-setup` instead of `setup-encrypted-backup.rb`. The early Keychain-passphrase reminder (printed right after `.shellrc` is sourced) now references the new default Keychain service name, `git-remote-gpg-encrypt` (was `gpg-encrypted-backup`).
...
* *[scripts/utilities/git_processor.rb, files/--HOME--/.shellrc]* Updated comments describing `clone_repo_into`/the Ruby delegate that previously claimed the encrypted-backup mechanism reused them internally -- the external tool is fully standalone (its own `git clone`/checkout logic) and does not call into either. `SKIP_POST_CLONE_MAINTENANCE`'s doc comment no longer cites the now-deleted `EncryptedBackup._ensure_wrapper_repo` as its motivating example (no current caller passes `skip_maintenance: true`; the mechanism is kept for any future ephemeral-clone use case with the same shape).
...
* Run `brew bundle install` to install the new tap/formula (also removes the direct `gnupg` dependency line, though the package itself stays installed transitively).
...
* Restart Terminal/iTerm to reload `.shellrc`'s renamed env vars.
