...
After symlinking, `install-dotfiles.rb` ensures the line `Include "${SSH_CONFIGS_DIR}/global_config"` is present in `${SSH_CONFIGS_DIR}/config`. This is a post-symlink step — do not add it manually or duplicate the guard elsewhere.
...
Autoload functions (`push`, `pull`, `cc`, `upreb`, etc.) support per-project overrides. The public function is a thin wrapper:
