...
## 14. Adding an Encrypted Backup (gpg + git bundle) Alongside Keybase

This repo's own Keybase *identity/setup* code has been removed (`scripts/utilities/keybase.rb` and every call site are gone) -- but that's not the same as removing Keybase itself: if `~` or `${PERSONAL_PROFILES_DIR}` still has a live `keybase://` `origin` remote, it keeps being pushed to normally by plain `git push` (no special dotfiles code needed for that beyond Keybase/KBFS being installed and running). What's new is a second, independent backup mechanism running alongside it: `git bundle` + `gpg --symmetric`, implemented in `scripts/utilities/encrypted_backup.rb`. Both run on every push -- see item 5 below -- so you always have two addressable restore points to choose between on a new machine.
...
- `scripts/migrate-repo-to-encrypted-backup.rb` -- **one-time** setup tooling for existing repos (run once per repo, to add the encrypted backup alongside any existing remote such as `keybase://`; `run_all` sets up all known repos in one call, `run` sets up a single arbitrary repo)
